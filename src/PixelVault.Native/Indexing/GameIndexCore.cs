...
namespace PixelVaultNative
{
    public sealed partial class MainWindow
    {
        ...
        string StripKnownPlatformSuffixes(string value)
        {
            ...
            while (true)
            {
                var updated = Regex.Replace(cleaned, @"\s*-\s*(Steam|Emulation|PS5|PlayStation|Nintendo Switch|Switch|Nintendo|Xbox PC|Xbox\/Windows|Xbox Windows|Xbox|PC)\s*$", string.Empty, RegexOptions.IgnoreCase);
        ...
        GameIndexEditorRow CloneGameIndexEditorRow(GameIndexEditorRow row)
        {
            ...
            return new GameIndexEditorRow
            {
                ...
                IgdbId = CleanTag(row.IgdbId),
                IgdbSlug = CleanTag(row.IgdbSlug),
                IgdbCollectionId = CleanTag(row.IgdbCollectionId),
                IgdbCollectionName = CleanTag(row.IgdbCollectionName),
                IgdbFranchiseId = CleanTag(row.IgdbFranchiseId),
                IgdbFranchiseName = CleanTag(row.IgdbFranchiseName),
                IgdbSummary = row.IgdbSummary ?? string.Empty,
                IgdbReleaseDate = CleanTag(row.IgdbReleaseDate),
                IgdbGenres = row.IgdbGenres ?? string.Empty,
                IgdbPlatforms = row.IgdbPlatforms ?? string.Empty,
                IgdbDeveloper = row.IgdbDeveloper ?? string.Empty,
                IgdbPublisher = row.IgdbPublisher ?? string.Empty,
                IgdbCoverImageId = CleanTag(row.IgdbCoverImageId),
                IgdbHiddenSeriesIds = row.IgdbHiddenSeriesIds ?? string.Empty,
                IgdbFetchedUtcTicks = row.IgdbFetchedUtcTicks > 0 ? row.IgdbFetchedUtcTicks : 0L,
        ...
        List<GameIndexEditorRow> MergeGameIndexRows(IEnumerable<GameIndexEditorRow> rows)
        {
            ...
            foreach (var group in normalizedRows.GroupBy(BuildGameIndexMergeKey, StringComparer.OrdinalIgnoreCase))
            {
                ...
                mergedRows.Add(new GameIndexEditorRow
                {
                    ...
                    IgdbId = groupRows.Select(row => row.IgdbId ?? string.Empty).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty,
                    IgdbSlug = groupRows.Select(row => row.IgdbSlug ?? string.Empty).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty,
                    IgdbCollectionId = groupRows.Select(row => row.IgdbCollectionId ?? string.Empty).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty,
                    IgdbCollectionName = groupRows.Select(row => row.IgdbCollectionName ?? string.Empty).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty,
                    IgdbFranchiseId = groupRows.Select(row => row.IgdbFranchiseId ?? string.Empty).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty,
                    IgdbFranchiseName = groupRows.Select(row => row.IgdbFranchiseName ?? string.Empty).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty,
                    IgdbSummary = groupRows.Select(row => row.IgdbSummary ?? string.Empty).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty,
                    IgdbReleaseDate = groupRows.Select(row => row.IgdbReleaseDate ?? string.Empty).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty,
                    IgdbGenres = groupRows.Select(row => row.IgdbGenres ?? string.Empty).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty,
                    IgdbPlatforms = groupRows.Select(row => row.IgdbPlatforms ?? string.Empty).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty,
                    IgdbDeveloper = groupRows.Select(row => row.IgdbDeveloper ?? string.Empty).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty,
                    IgdbPublisher = groupRows.Select(row => row.IgdbPublisher ?? string.Empty).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty,
                    IgdbCoverImageId = groupRows.Select(row => row.IgdbCoverImageId ?? string.Empty).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty,
                    IgdbHiddenSeriesIds = groupRows.Select(row => row.IgdbHiddenSeriesIds ?? string.Empty).FirstOrDefault(value => !string.IsNullOrWhiteSpace(value)) ?? string.Empty,
                    IgdbFetchedUtcTicks = groupRows.Select(row => row.IgdbFetchedUtcTicks).Where(t => t > 0).DefaultIfEmpty(0L).Max(),
