    ...
    public sealed partial class MainWindow
    {
        ...
        internal static ICoverService CreateCoverService(MainWindow mw, IFileSystemService fileSystem, string coversRoot)
        {
            return new CoverService(new CoverServiceDependencies
            {
                ...
                GetRetroAchievementsWebApiKey = delegate { return mw.CurrentRetroAchievementsApiKey(); },
        ...
        internal static IMetadataService CreateMetadataService(MainWindow mw, string cacheRoot)
        {
            return new MetadataService(new MetadataServiceDependencies
            {
                ...
                GetMetadataWorkerCount = ImportWorkflowOrchestration.GetMetadataWorkerCount,
                Log = delegate(string message) { mw.Log(message); },
            ...
            IGameIndexEditorAssignmentService gameIndexEditorAssignmentService)
        {
            return new ImportServiceDependencies
            {
                ...
                NormalizeGameIndexNameWithFolder = (name, folderPath) => mw.NormalizeGameIndexName(name, folderPath),
                LoadSavedGameIndexRows = root => mw.GetSavedGameIndexRowsForRoot(root) ?? new List<GameIndexEditorRow>(),
                BuildGameIndexTitleCounts = rows => mw.BuildGameIndexTitleCounts(rows),
                BuildGameIndexIdentity = (name, platformLabel) => mw.BuildGameIndexIdentity(name, platformLabel),
