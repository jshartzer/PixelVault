...
- Status (Mar 2026) — **`LoadGameIndexEditorRowsCore`** uses **`ILibraryScanner.EnsureGameIndexFolderContext`** (cached folders + forced rebuild when empty + log via host). Library browser folder refresh uses **`ILibrarySession.LoadLibraryFoldersCached`**. **`SaveGameIndexEditorRows`** uses **`ILibrarySession.LoadLibraryFoldersCached(false)`** when the save **`root`** matches **`librarySession.LibraryRoot`**; otherwise **`ILibraryScanner.LoadLibraryFoldersCached(root, false)`** for alias alignment. **`IImportService.FinalizeManualMetadataItemsAgainstGameIndex`** owns the manual-metadata **finish** loop; **`MainWindow`** still supplies resolve/platform/grouping/save funcs via **`ImportServiceDependencies`** until a dedicated game-index service absorbs them. **Follow-up (Apr 2026):** **`GetSavedGameIndexRowsForRoot`**, startup preload, folder ID editor, library metadata apply upsert, manual-metadata finish title rows, and Steam AppID / SteamGridDB resolve helpers now route through **`ILibrarySession`** when the active library root matches; remaining direct **`indexPersistenceService`** / **`libraryScanner`** use is mostly other roots, indexing partials, filename rules, and editors — narrow those only when a change already touches those files.
- **Phase 1a–1c (Apr 2026):** Game index editor + **`GetSavedRowsForRoot`** + routed reads (**scanner bridge**, **filename parser**, alignment / intake / metadata-edit paths) through **`GetSavedGameIndexRowsForRoot`**; **`LibrarySession`** persistence delegate unchanged (no recursion).
- **Phase 2 (Apr 2026):** **`LibraryBitmapLruCache`**/**`LibraryImageLoadCoordinator`** + **`LibraryThumbnailPipeline`** (decode/cache/poster I/O); **`MainWindow.LibraryImageLoading.cs`** keeps **`QueueImageLoad`** and thin forwards to the pipeline.
...
- Status (Apr 2026): **`LibraryBrowserHost`** (`UI/Library/LibraryBrowserHost.cs`) is the **sole show entry** (try/catch, logging, **`ILibrarySession`**). Library open/show wiring lives in **`MainWindow.LibraryBrowserShowOrchestration`**, which takes **`ILibraryBrowserShell`** only; **`MainWindow.LibraryBrowserShellBridge`** forwards to **`MainWindow`** partials (**`UI/Library/ILibraryBrowserShell.cs`**, **`MainWindow.LibraryBrowserShellBridge.cs`**). Per-open mutable state: **`LibraryBrowserWorkingSet`** (**`MainWindow.LibraryBrowserWorkingSet.cs`**, **`internal`**). Folder list / detail rendering: **`LibraryBrowserRenderFolderList`** / **`LibraryBrowserRenderSelectedFolderDetail`** (**`MainWindow.LibraryBrowserRender.*.cs`**). Folder tile + selection: **`LibraryBrowserBuildFolderTile`** / **`LibraryBrowserShowSelectedFolder`** (**`MainWindow.LibraryBrowserOrchestrator.FolderTile.cs`**). Top nav + window shell: **`GetOrCreateLibraryBrowserWindow`** / **`BuildLibraryBrowserNavChrome`** (**`MainWindow.LibraryBrowserChrome.cs`**). Folder/detail columns: **`BuildLibraryBrowserContentPanes`** (**`MainWindow.LibraryBrowserLayout.cs`**). **E2 slice:** **`ILibrarySession`** / **`LibrarySession`**. **Apr 2026:** pane resize/search/scroll wiring → **`MainWindow.LibraryBrowserOrchestrator.PaneEvents.cs`**; nav chrome + pane toolbar clicks → **`MainWindow.LibraryBrowserOrchestrator.NavChromeAndToolbar.cs`**. Next: further delegate blocks (sort/group setup, cover refresh hooks) only when Library orchestration is edited again.
...
11. ~~Library UI extraction (item 10)~~ — landed (**`LibraryBrowserHost.Show`** + **`LibraryBrowserShowOrchestration`** + **`ILibraryBrowserShell`** / **`LibraryBrowserShellBridge`**); Phase **E** roadmap complete for current scope; optional deeper splits when Library changes
