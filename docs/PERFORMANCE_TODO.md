...
- Status (Mar 2026): **`LibraryBrowserHost`** (`UI/Library/LibraryBrowserHost.cs`) is the entry point (receives **`ILibrarySession`**). **`MainWindow.ShowLibraryBrowserCore`** lives in **`UI/Library/MainWindow.LibraryBrowserOrchestrator.cs`** (dedicated partial file for Library browser UI; same **`MainWindow`** type, no behavior change). Top nav + window shell: **`GetOrCreateLibraryBrowserWindow`** / **`BuildLibraryBrowserNavChrome`** (**`MainWindow.LibraryBrowserChrome.cs`**). Folder/detail columns (**search, sort, virtualizers, banner, screenshots strip**): **`BuildLibraryBrowserContentPanes`** (**`MainWindow.LibraryBrowserLayout.cs`**). **E2 slice:** **`ILibrarySession`** / **`LibrarySession`** surface workspace + scanner + file seam + root. Next: optional further shrink (folder/detail layout, named handlers) when Library UI is touched again.
