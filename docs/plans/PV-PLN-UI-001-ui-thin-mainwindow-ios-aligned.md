...
| Metric | Apr 6 | Apr 9 | Apr 12 | Apr 17 (plan v2) | **Apr 17 (Step 8A)** | Direction |
|--------|------:|------:|-------:|------:|------:|-----------|
| `PixelVault.Native.cs` lines | ~2.9k → ~2.1k | ~2.1k | ~1.9k | 2,058 | **1,262** | −38.7 % in one move; Steps 8B → 9 → 10 → 11 → 12 expected to cut another ~400 |
| `MainWindow` partial count | ~40 | ~52 | ~54 | ~55 | **~56** | +1 for `MainWindow.LibraryCoverResolution.cs`; may grow by 2–3 as Steps 8B–13 replace monolith lines |
| Largest non-monolith partial (lines) | — | — | — | 1,088 (`LibraryBrowserViewModel`) | **1,088** (same; `LibraryCoverResolution` = 817) | Step 13 target |
| New domain logic in `MainWindow` | Discouraged | — | — | Still discouraged | Still discouraged | **Zero** for new features per `ios_foundation_guide` checklist |
