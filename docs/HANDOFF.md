...
- **Phase 5 (import):** Import-and-edit **Steam store title** when the user leaves the loaded title unchanged — **`IImportService.ApplyImportAndEditSteamStoreTitlesWhenGameNameUnchangedAsync`** (**`ICoverService.SteamNameAsync`**). Manual metadata **finish** — **`IImportService.FinalizeManualMetadataItemsAgainstGameIndex`** uses **`IGameIndexEditorAssignmentService`** for row resolve + persist ( **`ImportServiceDependencies.GameIndexEditorAssignment`** ). **`RunSteamRename`** uses **`SteamNameAsync`** when **`ResolveSteamStoreTitle`** is not set. Unit tests: **`tests/PixelVault.Native.Tests/ImportServiceManualMetadataTests.cs`**.
- **Phase E2 (library session):** **`ILibrarySession.PersistGameIndexRows`**; **`MainWindow.LibraryBrowser`** uses it after embedded-metadata repair instead of calling **`SaveSavedGameIndexRows`** with **`librarySession.LibraryRoot`** directly.
- **Publish:** script copies full native + test sources under `dist/.../source/`

Next likely slices: extend **`ILibrarySession`** with more read paths (e.g. metadata index load) if Library partials keep growing, route remaining **`LoadLibraryMetadataIndex`**-style calls through session or scanner where it clarifies ownership, trim dead **`MainWindow`** cover sync shims (**`TryResolveSteamAppId`**, etc.) if nothing calls them, or further **`await`** cleanup per `PERFORMANCE_TODO.md` item 11. **F1:** **`ShowSettingsWindow`** on **`MainWindow.SettingsShell`**. **Game index:** **`IGameIndexEditorAssignmentService`** is the current seam for import finalize + shared save/resolve (see **`GameIndexEditorAssignmentService.cs`**).
