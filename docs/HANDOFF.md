...
- **Library covers (UI thread):** Removed sync **`ResolveLibraryArt`**; tiles use **`GetLibraryArtPathForDisplayOnly`**; folder-detail banner art uses **`Task.Run`** + **`ResolveLibraryArtAsync(..., false)`** + dispatcher **`QueueImageLoad`** (same paths as before when downloads are off).
- **Publish:** script copies full native + test sources under `dist/.../source/`

Next likely slices: optional **`ILibrarySession`** facades for other **`librarySession.LibraryRoot`**-scoped reads only where it clarifies ownership; further import finish helpers on **`IImportService`** if **`ShowManualMetadataWindow`** stays bulky; or further **`await`** cleanup per `PERFORMANCE_TODO.md` item 11. **F1:** **`ShowSettingsWindow`** on **`MainWindow.SettingsShell`**. **Game index:** **`IGameIndexEditorAssignmentService`** is the current seam for import finalize + shared save/resolve (see **`GameIndexEditorAssignmentService.cs`**).
