...
- **Phase E2 (library session):** **`ILibrarySession.PersistGameIndexRows`** and **`ILibrarySession.LoadLibraryMetadataIndex`**; **`MainWindow.LibraryBrowser`** uses them for detail render and after embedded-metadata repair instead of reaching for **`SaveSavedGameIndexRows`** / **`LoadLibraryMetadataIndex`** on **`MainWindow`** directly where the session root applies.
- **Publish:** script copies full native + test sources under `dist/.../source/`

Next likely slices: route more Library reads through **`ILibrarySession`** only where **`LibraryRoot`** is the right scope (scanner / **`ILibraryScanHost`** still own arbitrary-root scan work), further import finish helpers on **`IImportService`** if **`ShowManualMetadataWindow`** stays bulky, or further **`await`** cleanup per `PERFORMANCE_TODO.md` item 11. **F1:** **`ShowSettingsWindow`** on **`MainWindow.SettingsShell`**. **Game index:** **`IGameIndexEditorAssignmentService`** is the current seam for import finalize + shared save/resolve (see **`GameIndexEditorAssignmentService.cs`**).
