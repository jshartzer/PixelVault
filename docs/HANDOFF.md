...
Next likely slices: more **`ShowManualMetadataWindow`** orchestration behind **`IImportService`** only when it stays pure (no UI in the service); **`PERFORMANCE_TODO.md`** item 10 (**`ShowLibraryBrowserCore`** shrink / named types) when Library UI is next touched; or further **`await`** cleanup per item 11. **F1:** **`ShowSettingsWindow`** on **`MainWindow.SettingsShell`**. **Game index:** **`IGameIndexEditorAssignmentService`** is the current seam for import finalize + shared save/resolve (see **`GameIndexEditorAssignmentService.cs`**).
