...
- **F1 (complete):** Path grid helpers in **`UI/Settings/MainWindow.SettingsShell.cs`**. **F2 (complete):** photography gallery index/cache + tag scan in **`UI/Photography/MainWindow.PhotographyAndSteam.cs`**; Steam match picker takes explicit **`Window`** owner; gallery errors **`LogException`**
