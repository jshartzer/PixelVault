...
- **Phase E2 (library session):** **`ILibrarySession`** surfaces **`PersistGameIndexRows`**, **`LoadLibraryMetadataIndex`**, **`LoadSavedGameIndexRows`**, **`SaveLibraryMetadataIndex`**, **`LoadLibraryFolderCacheSnapshot`**, **`HasLibraryFolderCacheSnapshot`**, **`ResolveIndexedLibraryDate`**, and **`BuildResolvedLibraryMetadataIndexEntry`** for **`LibraryRoot`**; **`MainWindow.LibraryBrowser`** detail render / prefill use **`librarySession`** instead of threading **`LibraryRoot`** into those **`MainWindow`** helpers where the active library applies.
...
Next likely slices: thin **`ILibrarySession`** wrappers for any remaining **`librarySession.LibraryRoot`** + **`MainWindow`** pairs in Library UI (e.g. **`RefreshLibraryCoversAsync`**, **`ShowLibraryMetadataScanWindow`**) only when it clarifies ownership; further import finish helpers on **`IImportService`** if **`ShowManualMetadataWindow`** stays bulky; or further **`await`** cleanup per `PERFORMANCE_TODO.md` item 11. **F1:** **`ShowSettingsWindow`** on **`MainWindow.SettingsShell`**. **Game index:** **`IGameIndexEditorAssignmentService`** is the current seam for import finalize + shared save/resolve (see **`GameIndexEditorAssignmentService.cs`**).
