...
- **Bundled tools (§5.9):** Repo **`tools-licenses\`** (GPLv3 + LGPL 2.1 full texts + README) merged into **`tools\licenses\`** on **`Publish-PixelVault.ps1`**, **`Publish-Velopack.ps1`**, and **`Build-PixelVault-AppTesting.ps1`** via **`Merge-BundledToolLicenses.ps1`** — replace COPYING files if your shipped **`ffmpeg.exe`**/**`exiftool.exe`** builds use different licenses (**`docs/BUNDLED_TOOLS_REDISTRIBUTION.md`**).
