...
- **Engineering / async cover paths:** **`EnrichLibraryFoldersWithSteamAppIds`** still completes via **`Task`** / **`GetAwaiter().GetResult()`** where applicable. **`ResolveLibraryArt`** sync wrapper removed; Library folder tiles use **`GetLibraryArtPathForDisplayOnly`** (custom / cache / preview, no download); the selected-folder banner resolves via **`Task.Run`** + **`await ResolveLibraryArtAsync(..., false)`** then marshals **`QueueImageLoad`** on the library window dispatcher. Unused **`MainWindow`** sync wrappers over **`coverService`** (**`TryDownloadSteamCover`**, **`TryDownloadSteamGridDbCover`**, **`ResolveBestLibraryFolderSteamAppId`**, **`ResolveBestLibraryFolderSteamGridDbId`**, SteamGrid ID resolve-by-name shims) were removed earlier. **`ICoverService`** sync methods still delegate into **`*Async`**.
