...
- **MainWindow extraction / Phase F:** **F1** — settings path grid helpers (**`SettingsTextBox`**, **`SettingsBrowseButton`**, **`PreferredSettingsWindowHeight`**) colocated in **`UI/Settings/MainWindow.SettingsShell.cs`**. **F2** — photography gallery index/cache and tag scan (**`GetTaggedImagesCached`** and helpers) plus Steam picker **`ShowSteamAppMatchWindow(Window owner, …)`** and gallery **`LogException`** in **`UI/Photography/MainWindow.PhotographyAndSteam.cs`**. **F3** — **`CaptureAppSettings`**, **`ApplyAppSettings`**, **`LoadSettings`**, **`SaveSettings`** moved to **`UI/Settings/MainWindow.SettingsPersistence.cs`** (still **`MainWindow`** partial; no behavior change). See **`docs/MAINWINDOW_EXTRACTION_ROADMAP.md`** Phase **F**.
