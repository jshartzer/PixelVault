...
- **Architecture (PV-PLN-EXT-002 A.2):** **`MainWindow.SettingsState.cs`** owns **`AppSettings`** ↔ window field mapping; **`MainWindow.SettingsPersistence.cs`** is load/save/notify only; **`ISettingsService`** documents single ini persistence path.
- **Settings:** **`import_move_conflict_mode`** (`Rename` / `Skip` / `Overwrite`) persisted in **`PixelVault.settings.ini`** via **`AppSettings.ImportMoveConflictMode`** and **`SettingsService.NormalizeImportMoveConflictMode`**.
- **Architecture (PV-PLN-EXT-002):** Documented where **`PixelVault.settings.ini`** may be touched (app **`SettingsService`** / migration / layout path only). **`ImportWorkflow.RunWorkflow`** uses a single **`BuildSourceInventory`** call per run (removed duplicate).
- **Performance (PV-PLN-PERF-001):** Intake preview, foreground import, manual intake, and headless import now share **`IntakePreparationBuilder`**, so review/manual/import-edit rows are built from one intake analysis dictionary; unit coverage locks this in and verifies HDR PNG/JXR alternates stay out of import and rename scopes.
- **Performance (PV-PLN-PERF-001):** Added the touched-path folder-cache update seam (`TryUpdateLibraryFolderCacheForTouchedPaths`) so import/edit call sites can refresh only affected game/orphan rows, using the previous cache snapshot plus current metadata index, instead of triggering a full-library cache rebuild; regression coverage verifies a touched game row updates without invoking the rebuild hook.
- **Performance (PV-PLN-PERF-001):** File metadata upserts, manual metadata upserts, metadata removals, and photo-index saves now try the touched-path folder-cache refresh first and fall back to a full rebuild only when needed; import sorting benefits through its existing scanner upsert path.
- **Performance (PV-PLN-PERF-001):** Explicit refresh paths remain intentional full refreshes: game-index refresh still uses the rebuild path, and `forceRefresh` folder loads ignore stale cache snapshots and rebuild from the current disk/index projection.
- **Performance (PV-PLN-PERF-001):** Added end-to-end guardrail coverage for import sort: a newly sorted root-level capture updates its cached game folder row without invoking the full-library rebuild hook.
- **Performance (PV-PLN-PERF-001):** Folder-cache rebuilds now split fast index projection from metadata repair. Missing/incomplete rows are queued for background repair, letting indexed folder rows save/display before embedded metadata reads complete; repaired entries refresh touched cache rows afterward.
- **Performance (PV-PLN-PERF-001):** Cache-miss library loads are covered for fast paint: usable folder rows are returned and cached before blocked metadata repair completes, then background repair updates the touched cache row.
- **Performance (PV-PLN-PERF-001):** The child-mtime-only cache path is now explicit as an index-only projection refresh: it rebuilds folder rows from persisted metadata-index paths and avoids a recursive folder sweep; tests lock the supplied indexed file list behavior.
- **Performance (PV-PLN-PERF-001):** Orphaned photo-index `GameId` rows are covered on the fast-paint path: they appear as pending assignment without blocking folder list render, then background metadata repair resolves them back to saved game rows and updates the touched folder-cache row.
- **Performance (PV-PLN-PERF-001):** Import workflow progress now routes through a 100ms coalescer so high-volume imports update the progress UI with the newest detail instead of dispatching once per file; pending progress flushes before cancel/success/failure messages.
- **Performance (PV-PLN-PERF-001):** Shared progress-window logs now batch text rendering at dispatcher background priority, preserving max-line trimming while avoiding a full `TextBox.Text` rebuild for every appended progress line.
- **Performance (PV-PLN-PERF-001):** High-volume import detail logs now batch main-log writes (`AppendMainLines`) for routine per-file move/sort/delete/rename/metadata lines, while summaries and errors still write immediately.
- **Performance (PV-PLN-PERF-001):** Library detail renders now own a per-render cancellation token. Changing selection cancels stale detail background work, deferred metadata repair checkpoints, and selection-dependent embedded metadata reads before they can continue into outdated UI updates.
- **Performance (PV-PLN-PERF-001):** Detail-pane embedded metadata now loads viewport/overscan captures first and defers the rest in small cancelable chunks, so opening a large folder no longer has to read metadata for every capture before the first useful refinement.
- **Performance (PV-PLN-PERF-001):** Stale detail-render guardrails now use one active-token/version/selection check. Deferred metadata repair carries the original render version through its background work and idle redraw callback so an older folder render cannot repaint after selection changes.
- **Performance (PV-PLN-PERF-001):** Virtualized folder/detail rows now coalesce ordinary scroll refreshes onto the next render pass instead of waiting on a debounce timer. Large page-style jumps still refresh immediately, which keeps fast navigation responsive without rebuilding rows for every wheel tick.
- **Performance guardrails (PV-PLN-PERF-001):** Added tests around virtual row element recycling and visible-row image decode prioritization so scroll tuning keeps reusing row visuals and still sends viewport-intersecting detail images through the priority load lane.
- **Diagnostics (PV-PLN-PERF-001):** Troubleshooting logging now records virtualized row scroll/rebuild diagnostics for the folder and detail hosts, making large-folder scroll passes easier to verify (`VirtualizedRowHostScroll`, `VirtualizedRowHostRowsRebuilt`).
- **Performance (PV-PLN-PERF-001):** SteamGridDB asset picker thumbnails/previews now download and decode off the UI thread, use frozen in-memory bitmaps, cache recent preview sizes, and prefer SteamGridDB preview URLs while browsing. The full image is still downloaded only when the user chooses an asset.
- **Performance (PV-PLN-PERF-001):** Custom cover/banner/logo saves from SteamGridDB picker selections and right-click art menus now perform disk copy/cleanup work off the UI thread, reducing stalls after choosing art.
- **Docs / QA (PV-PLN-PERF-001):** Final performance-plan doc sync now includes an isolated WPF manual smoke. App launch, 25/100/500-file foreground imports, large-folder detail open, fast detail scroll, and selection-switch diagnostics passed; the exported evidence lives in **`docs/perf/PV-PLN-PERF-001-step-29-manual-results.md`**. HDR PNG/JXR pair import plus SteamGridDB/custom-art live saves remain RC follow-ups with automated guardrails in place.
- **Performance:** Large folders with JPEG XR (`.jxr`) captures no longer probe every JXR file through WPF's slow dimension decoder before first paint. Detail layout uses a fast 16:9 screenshot aspect fallback for JXR files, while visible thumbnails still decode normally.
- **Docs / QA (PV-PLN-PERF-001):** Manual performance checks now include progress-window cancel responsiveness and large-import log completeness notes for 100-file and 500-file import runs.
- **Import:** **`ImportWorkflowOrchestration.CombineRenameStepResults`** aggregates Steam + manual **`RenameStepResult`** for unified import-and-comment (path maps merged); covered by **`ImportWorkflowOrchestrationProgressTests`**.
- **Import:** Game title cleanup now removes registered trademark / legal mark symbols such as **`®`**, **`™`**, HTML **`&reg;`**, and common mojibake variants before titles become import folders or Game Index names, so titles like **`Diablo® IV`** merge with **`Diablo IV`** instead of splitting.
- **Import:** **`SaveUndoAndSortAfterImportMoveIfNeeded`** in **`ImportWorkflow`** consolidates undo manifest + destination sort after a successful move (standard, unified, manual intake); sort step uses **`IImportService.SortDestinationRootIntoGameFolders`** directly (no UI-only **`SortDestinationFoldersCore`** wrapper).
- **Architecture (PV-PLN-EXT-002 A.4):** **`ILibraryScanHost`** contract documentation (grouped concerns, implementation pointer); **`LibraryScanHost`** summary in **`MainWindow.LibraryScannerBridge.cs`**.
- **Architecture (PV-PLN-EXT-002 A.5):** **`ImportWorkflow`** uses **`IFileSystemService`** for destination folder creation and **`File.Exists`**-equivalent checks (import / manual intake entry paths and unified move batch filtering).
- **Architecture (PV-PLN-EXT-002 A.5):** **`ImportWorkflowOrchestration`** progress totals and unified / manual-intake plans take **`IFileSystemService`**; **`MainWindow.ImportWorkflow.Steps`** uses the seam for metadata existence and timestamp capture plus **`RunMove`** filtering; headless standard import (**`HeadlessImportCoordinator`**, **`MainWindow.HeadlessImport`**) uses the same seam for mkdir, eligibility, and move filtering. **`IFileSystemService`** adds **`GetCreationTime`** (with **`FileSystemServiceTests`** coverage).
- **Docs (PV-PLN-EXT-002 B.3):** **`SERVICE_OWNERSHIP_AND_PARALLEL_WORK_MAP.md`** — composition-at-a-glance table (including **session log** after A.6), refreshed **`IFileSystemService`** / **`ISettingsService`** notes, **`ILogService`** candidate row → shipped v1.
- **Docs (PV-PLN-EXT-002):** Plan marked **Complete (Apr 2026)** — Phase **A–B** closure, §9 follow-on pointers (**`NEXT_TRIM_PLAN`**, **`PERFORMANCE_TODO`**, **`PV-PLN-DIST-001`**, incremental **`IFileSystemService`**); **`HANDOFF.md`** read-first + recent-progress pointers updated; **`docs/completed-projects/README.md`** index row.
- **Architecture (PV-PLN-EXT-002 A.6):** **`ILogService`** with **`TroubleshootingLogService`** (wraps **`TroubleshootingLog.AppendMainLine`**) and **`NullLogService`** for tests; **`MainWindowServiceGraph`** exposes **`LogService`**; **`MainWindow.Log`** uses the seam then mirrors to **`logBox`**; **`ImportServiceDependencies.LogService`** replaces the old optional **`Action<string> Log`** for **`ImportService`** session lines (**`LogServiceTests`**).
- **Architecture (PV-PLN-EXT-002 B.1):** Co-locate import-domain services under **`Services/Intake`** — moved **`ImportService.cs`**, **`ImportWorkflowOrchestration.cs`**, **`SteamImportRename.cs`** (namespace unchanged); updated **`PROJECT_CONTEXT`**, **`SERVICE_OWNERSHIP`**, **`REAL_APP_IMPLEMENTATION_MAP`**, **`pixelvault_service_split_plan.txt`**, **`PV-PLN-AINT-001`**, **`MAINWINDOW_EXTRACTION_ROADMAP`**, **`PV-PLN-UI-001`**, **`PV-PLN-RVW-001`** paths.
- **Tests:** **`LibraryPlatformLabelsTests`** expectations aligned with **`LibraryPlatformLabels.PlatformGroupOrder`** (**`Switch`** bucket and default **8** for unknown / **`PlayStation`**).
- **Architecture (PV-PLN-EXT-002 B.2):** **`IntakePipeline`** — thin facade over **`IImportService`**, **`IFileSystemService`**, and **`IntakeAnalysisService`** with **`RunStandardTopLevelSubsetAsync`** delegating to **`HeadlessImportCoordinator`**; **`MainWindowServiceGraph`** / **`MainWindow`** use **`intakePipeline`** instead of a standalone analysis field (**`IntakePreview`**, **`BackgroundIntake`**, **`HeadlessImport`**).
- **Docs (PV-PLN-EXT-002 B.3):** **`SERVICE_OWNERSHIP_AND_PARALLEL_WORK_MAP.md`** — mermaid **Data flow** diagram plus **Intake facade** composition row.
- **First-run paths:** Before any saved settings, **installed** builds default to **`C:\Game Capture Uploads`** / **`C:\Game Captures`** (not **`E:\`**). If the exe is running **under a local Codex repo** (e.g. **`C:\Codex`** dev or publish-from-repo), defaults are **`<repo>\Game Capture Uploads`** and **`<repo>\Game Captures`** so development stays inside the workspace.
- **Distribution (Phase 1):** **[Velopack](https://docs.velopack.io/)** installer/update plumbing — **`VelopackApp`** bootstrap in **`PixelVault.Native`**, **`scripts/Publish-Velopack.ps1`**, **`docs/VELOPACK.md`**. **`Publish-Velopack`** produces **self-contained** **`win-x64`** output with **`PublishSingleFile=false`** for **`vpk pack`**; **`Publish-PixelVault.ps1`** remains the daily **zip-style** **`dist/PixelVault-*`** publish (**framework-dependent** unless you change project defaults separately).
- **Installed / portable data roots:** **`PersistentDataMigrator`** honors **`PIXELVAULT_DATA_ROOT`** and **`PixelVault.data-root.ini`** (see **`docs/DISTRIBUTION_STORAGE.md`**); **Setup & health** surfaces the resolved app data folder.
- **Docs:** Privacy policy draft (**`docs/PRIVACY_POLICY.md`**); **EULA** draft (**`docs/EULA.md`** — replace **`{LEGAL_ENTITY}`** / **`{JURISDICTION}`** / **`{PUBLISH_CONTACT}`** before public release); roadmap **`PV-PLN-DIST-001`** updates (**§5.3** / **§5.5**).
- **Bundled tools (§5.9):** Repo **`tools-licenses\`** (**GPLv3** for **`exiftool.exe`** + README only) merged into **`tools\licenses\`** on publish scripts (**`Merge-BundledToolLicenses.ps1`**). **FFmpeg** is **optional** and **not** bundled; users install separately — **`SettingsService`** no longer resolves **`tools\ffmpeg.exe`** (**`docs/BUNDLED_TOOLS_REDISTRIBUTION.md`**).
- **Distribution docs:** **`docs/PUBLISH_SIGNING.md`** (Authenticode + Velopack signing); **`docs/PRIVACY_POLICY.md`** adds a **hosting** section for stable HTTPS (**§5.4**); roadmap **§10.1** — **legal hosting last** in Phase 1 priority order.
- **Velopack publish:** **`Publish-Velopack.ps1`** accepts **`-SignParams`** or **`VPK_SIGN_PARAMS`** for **`vpk pack -n`** (Authenticode during pack).
- **Distribution QA:** **`scripts/Verify-DistributionLayout.ps1`** validates **`PixelVault.exe`** + merged **`tools\licenses\`** before VM / golden-path runs; **`docs/SIMPLE_RELEASE_STEPS.md`** + **`scripts/Run-LocalReleaseChecks.ps1`** bundle the Velopack publish + verify flow for a single command.
...
- **Docs:** **`PV-PLN-RVW-001`** canonical plan moved to **`docs/completed-projects/`**; stubs and **`NEXT_TRIM_PLAN.md`** / **`CODE_QUALITY_IMPROVEMENT_PLAN.md`** / **`HANDOFF.md`** / **`APP_REVIEW_2026-04-12.md`** updated accordingly.

## 0.075.010
- **Release:** Version **0.075.010** — **`PV-PLN-LIBST-001`** (single-folder storage) **through Step 9**, **library scan / re-home** correctness, **`PV-PLN-AINT-001`** background **auto-intake** through **slice 9**, **library metadata** re-resolution for Steam / non-Steam shortcut mislabels, **filename parser** and **import** refinements, **JPEG XR** (`.jxr`), and **docs** updates.
- **LIBST / placement / photo index:** **`LibraryRehomeRules`**, **`LibraryPlacementService.IsCaptureAlreadyUnderCanonicalOrganizeTarget`**, organize skips moves when a capture is already under the canonical game folder (including subfolders). **`SavePhotoIndexEditorRows`** triggers **`RehomeLibraryCapturesTowardCanonicalFolders`** when **`GameId`** changes; after physical moves, **`photo_index`** stays aligned via **`RemoveLibraryMetadataIndexEntries`** (pre-move paths) + **`UpsertLibraryMetadataIndexEntries`** (same pattern as library metadata apply). **`LibraryScanner`** optional **`folderCacheRebuildHook`** for tests only.
- **Tests:** **`LibraryRehomeRulesTests`**, **`LibraryPlacementServiceTests`** (organize-target), **`PhotoIndexSaveRehomeIntegrationTests`** (fake host), **`PhotoIndexSaveRehomeSqliteIntegrationTests`** (real **`IndexPersistenceService`** SQLite + **`IndexPersistenceHarness`**). Plan **`PV-PLN-LIBST-001`** marked **complete** with Step 9 ops checklist (later archived — see below).
- **Library scan (nested captures):** Under each top-level folder inside the library root, media enumeration for **folder cache** and **metadata scan** uses **`SearchOption.AllDirectories`** so captures in **canonical subfolders** are indexed (matches organize “already under canonical” behavior). Scoped folder scans drop stale index keys for any path **under** the scan root, not only direct children.
- **Manual metadata / re-home naming:** When the photo index resolves a **saved game row**, **`ManualMetadataItem.GameName`** uses the **row title** only—no fallback to a conflicting **filename stem**—so **`OrganizeLibraryItems`** targets the correct canonical folder after **`GameId`** reassignment.
- **Background auto-intake (`PV-PLN-AINT-001`):** Optional **watcher-based** pipeline (quiet period, stability probe, headless import coordinator, foreground busy gate) moves eligible files into the normal library import path; **Settings** toggles; **Background imports** command-palette entry and **activity** window for review/undo with persisted batches. **Show summary after move** defaults **off**; when toasts are enabled they reference the command palette instead of auto-opening the activity window on every batch.
- **Library metadata scan / index merge:** **`ILibraryScanHost`** adds **`IndexEntryShouldReResolveForNonSteamShortcutMislabel`** and **`IndexEntryShouldReResolveSteamPlatformWithoutAppId`**; **`LibraryScanner`** re-merges embedded metadata when a row looks complete but platform/console labeling disagrees with filename parse + **Game Index** evidence. **`MetadataHelpers`** strips console-participating tags that would override **non-Steam shortcut** identity, drops **Steam** filename hints when the parse has no substantive Steam App ID, and threads shortcut-aware merges through **`LibraryMetadataIndexing`** (including **Steam** stored label without App ID evidence).
- **Filename parser / conventions:** Built-in **`emulation_nonsteam_shortcut_capture`**; **`IsCaptureHostPlaceholderGameTitle`** blocks capture-host window titles (e.g. **Dolphin**) from becoming game hints; **`NormalizeColonStandinUnderscoresForGameTitle`** also maps spaced **hyphen** subtitles to **`:`** (e.g. *Eternal Darkness - Sanity's Requiem*). **`.jxr`** is included in **`ext:image` / `ext:media`** patterns and related filename tooling.
- **Import:** For files parsed as **non-Steam shortcut** identity, metadata writes use **`Emulation`** platform tags even when the manual row carried other platform selections.
- **Formats:** **`.jxr`** (JPEG XR) is recognized as library **image** / **media** (indexing, **`IsImage` / `IsMedia`**), **Open file** filters, and **cover** asset discovery alongside existing raster extensions.
- **Docs:** Completed plan **`PV-PLN-LIBST-001-single-folder-storage-model.md`** moved to **`docs/completed-projects/`**; **`docs/plans/README.md`** and **`docs/completed-projects/README.md`** index updated.
- **Docs (planning):** **`PV-PLN-V1POL-001-pre-v1-polish-program.md`** — header **Status** and **Execution status** table aligned to shipped **`0.075.xxx`** work (slices **A–E** shipped; **F** largely shipped; **G/H/J** partial); grounded audit retained as **pre–`0.075`** baseline.
...
- **Startup:** Constructor-time game-index preload removed; saved rows load on **`DispatcherPriority.ApplicationIdle`** after the library browser is shown, improving first-frame contention.
- **HTTP / covers:** Sync download surface on **`TimeoutWebClient`** removed; cover and network paths stay **`async`**-only.
- **Docs:** **`docs/LIBRARY_PERFORMANCE_PLAN.md`** Steps **4** and **5** marked complete.

## 0.998.000
- **Release:** Version **0.998.000** — Library detail pane virtualization and decode scheduling (pre-1.0 train).
- **Versioning:** **`AppVersion`** and published **`dist`** folders now use the triple **`0.AAA.BBB`** scheme (this build maps the prior **`0.998`** train to **`0.998.000`**). See **`docs/POLICY.md`** and **`docs/VERSIONING.md`**.
- **Library / detail pane:** Image decodes for packed day-card rows use **viewport-aware** priority: rows that intersect the live **ScrollViewer** viewport enqueue on the **priority** lane; rows materialized only for virtual **overscan** use the **normal** lane (`LibraryDetailTileRowIntersectsViewport`, `CreateLibraryDetailTile` `prioritizeImageDecode`).
- **Library / detail virtualization:** **`DetailRows.RecycleVisibleRowElements`** is enabled like folder **`TileRows`**. After each visible-row rebuild, **`RepopulateLibraryDetailTilesFromVisibleRows`** walks the visible visual tree so **`DetailTiles`** matches on-screen tiles for selection chrome (build no longer pushes tiles into the list directly). See **`docs/LIBRARY_PERFORMANCE_PLAN.md`** Step 3.
...
- **MainWindow extraction / Phase F:** **F1** — settings path UI lives with settings flow. **F2** — photography gallery index/cache and **`ShowSteamAppMatchWindow(Window owner, …)`** in **`UI/Photography/MainWindow.PhotographyAndSteam.cs`**. **F3** — **`MainWindow.SettingsPersistence.cs`** for **`Capture`/`Apply`/`Load`/`Save`**. **`SettingsShellHost`** + **`SettingsShellDependencies`** own Settings + Path Settings modal UI; **`MainWindow.SettingsShell.cs`** builds deps and exposes **`ShowSettingsWindow`** / **`ShowPathSettingsWindow`**. See **`docs/MAINWINDOW_EXTRACTION_ROADMAP.md`** Phase **F**. No intentional behavior change.
- **Library / Phase E:** **`LibraryBrowserHost.Show`** is the sole Library open entry (logging + error UI); removed **`MainWindow.ShowLibraryBrowserCore`** and **`MainWindow.LibraryBrowserOrchestrator.cs`**. **`MainWindow.LogException`** is **`internal`** for host use. **`ILibraryBrowserShell`** + **`MainWindow.LibraryBrowserShellBridge`** decouple **`LibraryBrowserShowOrchestration`** from the **`MainWindow`** type (forwarding-only bridge; no behavior change). Docs updated for Phase **E** completion.
...
- **Import:** `RunSteamRenameAsync` on `IImportService` / `ImportService`; import and game-index progress hosts use `Func<..., Task<TResult>>` backgrounds (`Task.Run` + `await`) so Steam rename uses `ICoverService.SteamNameAsync` on the async path. Game index Resolve-IDs progress no longer uses `GetAwaiter().GetResult()` inside the background lambda.
...
- **Library (browser model):** Introduced a browser-only projection layer so merged game-first Library rows can share one timeline without changing scanner output, disk layout, or saved game-index persistence.
- **Library (merged-row guardrails):** In merged `All` mode, folder-level actions that still assume one concrete folder are intentionally constrained for now: the banner and context menu use **Open Primary Folder**, while custom-cover, cover-fetch, and ID-edit actions stay available only on non-merged rows.
- **Engineering / structure (Library UI):** **`MainWindow.ShowLibraryBrowserCore`** moved from **`UI/Library/MainWindow.LibraryBrowser.cs`** to **`UI/Library/MainWindow.LibraryBrowserOrchestrator.cs`** (same **`MainWindow`** partial, no intended behavior change). **`LibraryBrowserHost`** entry unchanged. **`docs/PERFORMANCE_TODO.md`** item 10 status updated.
- **Engineering / structure (Library UI, continued):** Library browser **window shell** and **top navigation bar** (import/settings/index buttons, refresh/fetch, intake badge) built by **`GetOrCreateLibraryBrowserWindow`** and **`BuildLibraryBrowserNavChrome`** in **`UI/Library/MainWindow.LibraryBrowserChrome.cs`**; orchestrator wires handlers only (no intended behavior change).
- **Engineering / structure (Library UI, continued):** **Folder list** (search, sort pills, folder virtualizer, status line), **splitter**, and **detail pane** (banner, screenshots header, detail virtualizer) built by **`BuildLibraryBrowserContentPanes`** in **`UI/Library/MainWindow.LibraryBrowserLayout.cs`** (**`LibraryBrowserPaneRefs`**); orchestrator keeps state and event wiring (no intended behavior change).
- **Engineering / structure (Library UI, continued):** Library folder grid render moved to **`LibraryBrowserRenderFolderList`** (**`MainWindow.LibraryBrowserRender.FolderList.cs`**, **`LibraryBrowserFolderListContext`**); detail pane render (**`Task.Run`** snapshot + metadata repair) moved to **`LibraryBrowserRenderSelectedFolderDetail`** (**`MainWindow.LibraryBrowserRender.DetailPane.cs`**, **`LibraryBrowserDetailPaneContext`**). Orchestrator syncs context fields before/after each call (no intended behavior change).
- **Engineering / structure (Phase E2 + import):** **`ILibrarySession`** adds **`EnsureLibraryRootAccessible`** (**`EnsureDir`**), **`HasLibraryRoot`**, **`RemoveLibraryMetadataIndexEntries`**, **`LoadLibraryFoldersCached`**, **`RefreshLibraryCoversAsync`**, **`RunLibraryMetadataScan`**, plus metadata index helpers and folder-cache snapshot; Library browser orchestration (**`ShowLibraryBrowserCore`**) no longer reads **`librarySession.LibraryRoot`** for those flows. Manual metadata finish applies tag-text platform hints and validates “Other” custom name via **`IImportService.ApplyManualMetadataTagTextToPlatformFlags`** / **`ManualMetadataItemsMissingOtherPlatformName`** (**`ImportServiceDependencies.ParseManualMetadataTagText`**, **`CleanTag`**). Finish dialog copy (empty selection, **Add New Game** prompt, final confirm) is **`GetManualMetadataFinishEmptySelectionMessage`**, **`BuildManualMetadataAddNewGamePrompt`**, **`GetManualMetadataFinishConfirmBody`**. Removed unused **`MainWindow`** cover shims (**`TryResolveSteamAppId`**, **`SearchSteamAppMatches`**, **`TryResolveSteamGridDbIdBySteamAppId`**, **`TryResolveSteamGridDbIdByName`**, sync **`ResolveBestLibraryFolderSteamAppId`** / **`ResolveBestLibraryFolderSteamGridDbId`**, sync **`TryDownloadSteamCover`** / **`TryDownloadSteamGridDbCover`**). Unit tests extended in **`ImportServiceManualMetadataTests`**.
