...
- **Engineering / structure (Phase E2 + import):** **`ILibrarySession.LoadLibraryMetadataIndex`** loads the photo metadata index for **`LibraryRoot`** (empty map when root unset); Library detail render uses **`librarySession.LoadLibraryMetadataIndex`** instead of calling **`MainWindow.LoadLibraryMetadataIndex`** directly. Manual metadata finish applies tag-text platform hints and validates “Other” custom name via **`IImportService.ApplyManualMetadataTagTextToPlatformFlags`** / **`ManualMetadataItemsMissingOtherPlatformName`** (**`ImportServiceDependencies.ParseManualMetadataTagText`**, **`CleanTag`**). Removed unused **`MainWindow`** sync cover shims **`TryResolveSteamAppId`** and **`SearchSteamAppMatches`**. Unit tests extended in **`ImportServiceManualMetadataTests`**.
...
- **Engineering / async cover paths:** Sync **`ResolveLibraryArt`**, **`TryDownloadSteamCover`**, **`TryDownloadSteamGridDbCover`**, **`ResolveBestLibraryFolderSteamAppId`**, **`ResolveBestLibraryFolderSteamGridDbId`**, and **`EnrichLibraryFoldersWithSteamAppIds`** delegate into the existing **`*Async`** implementations (single code path; same blocking when called synchronously).
