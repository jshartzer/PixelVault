...
- **Refactor (no intentional UI change):** [MainWindow extraction](docs/MAINWINDOW_EXTRACTION_ROADMAP.md) **Phase B** in source — `UI/ChangelogWindow.cs`, `UI/UiBrushHelper.cs`, `UI/Progress/WorkflowProgressWindow.cs` (`WorkflowProgressWindow.Create` for import progress, library metadata scan/apply, and cover refresh).
