...
- **Docs:** Privacy policy draft (**`docs/PRIVACY_POLICY.md`**); roadmap **`PV-PLN-DIST-001`** updates (**§5.3** / **§5.5**).
- **Bundled tools (§5.9):** Repo **`tools-licenses\`** (**GPLv3** full texts for **`exiftool.exe`** and **gyan.dev FFmpeg essentials GPL** builds + README) merged into **`tools\licenses\`** on **`Publish-PixelVault.ps1`**, **`Publish-Velopack.ps1`**, and **`Build-PixelVault-AppTesting.ps1`** via **`Merge-BundledToolLicenses.ps1`** — replace **`ffmpeg-*-COPYING.txt`** if you switch FFmpeg variant (**`docs/BUNDLED_TOOLS_REDISTRIBUTION.md`**).
