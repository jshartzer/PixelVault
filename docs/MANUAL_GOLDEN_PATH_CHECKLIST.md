...
| Large-folder scroll diagnostics | With **Settings → Diagnostics → Enable troubleshooting logging** on, open Diablo IV/Timeline/another hundreds-capture folder, fast-scroll the right-hand detail pane for 10-15 seconds, then switch folders while work is settling. | `PixelVault-troubleshooting.log` rows for `VirtualizedRowHostScroll`, `VirtualizedRowHostRowsRebuilt`, `LibraryDetailRenderSkipped`, and `LibraryDetailRenderApplied`. | Pass if `DetailRows` refreshes are mostly `mode=render-coalesced`, page-sized jumps are `mode=immediate-page-jump`, row ranges follow the scroll offset without obvious gaps, and stale renders are skipped rather than repainting the previous folder. |
