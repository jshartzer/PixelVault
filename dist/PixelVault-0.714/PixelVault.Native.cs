    ...
    public sealed class MainWindow : Window
    {
        ...
        void RunManualMetadata(List<ManualMetadataItem> items, Action<int, int, string> progress = null)
        {
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                item.ForceTagMetadataWrite = false;
                updated++;
                ...
                renderTiles = delegate(bool forceRefresh)
                {
                    ...
                    foreach (var folderGroup in folderGroups)
                    {
                        ...
                        foreach (var folder in folderGroup.OrderBy(f => f.Name))
                        {
                            ...
                            var tileArt = LoadImageSource(ResolveLibraryArt(folder, false), Math.Max(tileWidth * 3, 760));
                            if (tileArt != null) imageBorder.Child = new Image { Source = tileArt, Width = tileWidth, Height = tileHeight, Stretch = Stretch.UniformToFill };
